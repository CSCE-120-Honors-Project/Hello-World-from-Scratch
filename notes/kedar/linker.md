...
Symbols defined in a linker script can be used within the C/C++ program being linked using the `extern` keyword. For example:
```c
extern int x; // Tells the compiler that the variable x is defined elsewhere, like in the linker script
```
When using `extern` variables, always use the addressof (`&`) operator. The C compiler generates code that dereferences the `extern` address at runtime, and the values may be uninitialized or garbage.
